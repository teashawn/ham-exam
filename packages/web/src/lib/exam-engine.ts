...
export function buildExamQuestions(
  ...
  for (const section of examData.sections) {
    ...
    if (questionsToSelect > 0) {
      const selected = selectRandomQuestions(section.questions, questionsToSelect);
...
export function calculateSectionResult(
  ...
  const questionsInExam = sessionQuestions.filter((q) => sectionQuestionIds.has(q.id));
...
export function completeExam(session: ExamSession, examData: ExamData): ExamResult {
  ...
  const sectionResults = examData.sections
    .map((section) => calculateSectionResult(section, session.questions, session.answers))
